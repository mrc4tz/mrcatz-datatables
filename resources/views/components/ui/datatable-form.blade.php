...
@php
    // Optional modal DOM-id suffix so a single page component can host
    // multiple CRUD forms (one per datatable / pageName). Set it on the
    // consuming form partial BEFORE @extends('mrcatz::components.ui.datatable-form')
    // via `@php($modalSuffix = '-indikator')` — the same value expected by
    // datatable-scripts (mrcatzModalSuffix(pageName)). Defaults to '' for
    // single-CRUD pages so DOM ids stay byte-identical to pre-v1.29.22.
    $modalSuffix = $modalSuffix ?? '';
@endphp
@unless(($this->modalFullScreen ?? false))
<dialog id="modal-data{{ $modalSuffix }}" class="modal modal-bottom sm:modal-middle" wire:ignore.self aria-modal="true" aria-labelledby="modal-data-title{{ $modalSuffix }}">
    <div class="modal-box w-full max-w-3xl lg:max-w-4xl bg-base-100 rounded-t-2xl sm:rounded-2xl shadow-2xl" x-data x-trap.noscroll="document.getElementById('modal-data{{ $modalSuffix }}')?.open">
        {{-- Header --}}
        <div class="flex items-center justify-between pb-4 mb-4 border-b border-base-content/10">
            <h3 id="modal-data-title{{ $modalSuffix }}" class="text-lg font-bold text-base-content flex items-center gap-2">
    ...
    @else
        ...
        <script>
            (function () {
                const dlg = document.getElementById('modal-data{{ $modalSuffix }}');
...
<dialog id="modal-data-delete{{ $modalSuffix }}" class="modal modal-bottom sm:modal-middle" wire:ignore.self aria-modal="true" aria-labelledby="modal-delete-title{{ $modalSuffix }}">
    <div class="modal-box bg-base-100 rounded-t-2xl sm:rounded-2xl shadow-2xl" x-data x-trap.noscroll="document.getElementById('modal-data-delete{{ $modalSuffix }}')?.open">
        ...
        <h3 id="modal-delete-title{{ $modalSuffix }}" class="text-lg font-bold text-center text-base-content">{{ mrcatz_lang('confirm_delete') }}</h3>
    ...
    @else
        ...
        <script>
            (function () {
                const dlg = document.getElementById('modal-data-delete{{ $modalSuffix }}');
                if (!dlg || dlg.dataset.mrcatzGuarded === '1') return;
                dlg.dataset.mrcatzGuarded = '1';
                dlg.addEventListener('cancel', (e) => e.preventDefault());
            })();
